...
float a6 = -0.9;
float b6 = 0.58;
float w  = .4;  //removed *10 that was there for testing (might be used to slow down, remember 0.1879*10)
...
float amp_shift_deg = 0;  //20 deg amp_shift
...
int main() {
    ...
    int id[NUM_OF_MOTORS + 1] = {NUM_OF_MOTORS, 1, 2, 3};  //right, left
  ...
  //big comment STARTS here
    ...
    while(running_for <= run_time_ms) {
        ...
        if(elapsed >= dt_ms) {
            ...
            float theta_d_right_flipped = -theta_d_right;

            cout << theta_d_right_flipped << std::fixed << std::setprecision(3) << ";"; // DEBUG

            int rounded_theta = theta_d_right_flipped * 1000;
            itoa(rounded_theta, buf, 10);
            theta_str_n = buf + newline;
            writePort.write(theta_str_n);
            ...
            rounded_theta = theta_d_right_flipped * 1000;
            itoa(rounded_theta, buf, 10);
            theta_str_n = buf + newline;
            writePort.write(theta_str_n);
