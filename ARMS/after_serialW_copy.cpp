...
float a6 = -0.2732;
float b6 = 0.06696;
float w  = 0.65;  //removed *10 that was there for testing
  ...
  //big comment STARTS here
    ...
    while(running_for <= run_time_ms) {
        ...
        if(elapsed >= dt_ms) {
            ...
            // Change local directory depending where you have JSON file stored
            ifstream motor("C:\\Users\\Mo Sharifi\\Documents\\PlatformIO\\Projects\\Exoskeleton code\\serial-rw\\serialComm_WriteAndRead\\motor.json");
            json jf = json::parse(motor);
            string motorCaseString = jf["case"];
            string freqString = jf["w"];
            int motorCase = stoi(motorCaseString);
            float newFreq = stof(freqString);

            // Ramp-up configuration
            const int rampSteps = 10; // Number of steps to ramp between frequencies
            static int currentRampStep = 0; // Current step in the ramp process
            static float initialFreq = w; // Keep the initial frequency to start ramping from
            if (newFreq != w && currentRampStep == 0) {
                // If there's a new frequency and we're not already ramping, start the ramp
                initialFreq = w;
            }
            ...
            if (newFreq != w || currentRampStep > 0) {
                if (currentRampStep < rampSteps) {
                    // Ramping in progress
                    float freqDiff = newFreq - initialFreq;
                    w = initialFreq + (freqDiff * currentRampStep / rampSteps);
                    currentRampStep++;
                } else {
                    // Ramping complete, ensure the target frequency is exactly set
                    w = newFreq;
                    currentRampStep = 0; // Reset for the next ramp
                }
        }

            // Proceed with the motor case logic, using the potentially ramped frequency `w`
            if (motorCase > 0) {
                ...
                //Setting previous theta d value in case motor condition goes to paused state
            ...
            // Motorcase == 0 is when we have the paused state
            if (motorCase == 0) {
