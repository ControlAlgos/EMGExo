...
float a6 = -0.2732;
float b6 = 0.06696;
float w  = .95; //0.1879 * 10;  //removed *10 that was there for testing
float theta_d_right = 0;
float theta_d_left = 0;
float theta_d_right_ankle = 0;
float theta_d_left_ankle = 0;
...
float amp_shift_deg = 30;  //20 deg amp_shift
float amp_shift_deg_right_knee = 15;
float amp_shift_deg_left_knee = 5;
...
int main() {
    ...
    const int NUM_OF_MOTORS = 6;  //just the two hip motors for now
    int id[NUM_OF_MOTORS + 1] = {NUM_OF_MOTORS, 1, 2, 3, 4, 5, 6};  //right (hip), left (hip), right (knee), Left(knee) 
    ...
    //while(run_again[0] == 'y'){    //run_again is a c string, so it's a char array
        ...
        int dt = 15;    //this is an int; units in ms
...
// ===== Send 4th hip motor's ID ========================================================== //
                ...
                // ===== Send 1st hip motor's ID =========================================================== //
                itoa(id[5], buf, 10);  //use the id at index 1 of id array above
                id_str_n = buf + newline;
                writePort.write(id_str_n);
...
// ===== Send 3rd hip motor's ID (RIGHT KNEE) =================================================================== //
            ...
            ktheta_d_right = (-M_PI/180) * (ka0 + ka1*cos(t*w) + kb1*sin(t*w) + 
                ...
                ka6*cos(6*t*w) + kb6*sin(6*t*w)- amp_shift_deg_right_knee); //
...
// ===== Send 4th hip motor's ID (LEFT KNEE) ================================================================== //
            ...
            ktheta_d_left = (M_PI/180) * (ka0 + ka1*cos((t-b)*w) + kb1*sin((t-b)*w) + 
                ...
                ka6*cos(6*(t-b)*w) + kb6*sin(6*(t-b)*w)- amp_shift_deg_left_knee );//
...
// ===== Send 5th ankle motor's ID =================================================================== //
            itoa(id[5], buf, 10);  //use the id at index 1 of id array above
            id_str_n = buf + newline;
            writePort.write(id_str_n);

            theta_d_right_ankle = (M_PI/180) * (a0 + a1*cos(t*w) + b1*sin(t*w) + 
                a2*cos(2*t*w) + b2*sin(2*t*w) + a3*cos(3*t*w) + b3*sin(3*t*w) + 
                a4*cos(4*t*w) + b4*sin(4*t*w) + a5*cos(5*t*w) + b5*sin(5*t*w) +
                a6*cos(6*t*w) + b6*sin(6*t*w) - amp_shift_deg);

            cout << theta_d_right_ankle << std::fixed << std::setprecision(3) << ";"; // DEBUG

            rounded_theta = theta_d_right_ankle * 1000;
            itoa(rounded_theta, buf, 10);
            theta_str_n = buf + newline;
            writePort.write(theta_str_n);
            

// ===== Send 6th ankle motor's ID ================================================================== //
            itoa(id[6], buf, 10);  //use the id at index 1 of id array above
            id_str_n = buf + newline;
            writePort.write(id_str_n);

            theta_d_left_ankle = (-M_PI/180) * (a0 + a1*cos((t-b)*w) + b1*sin((t-b)*w) + 
                a2*cos(2*(t-b)*w) + b2*sin(2*(t-b)*w) + a3*cos(3*(t-b)*w) + b3*sin(3*(t-b)*w) + 
                a4*cos(4*(t-b)*w) + b4*sin(4*(t-b)*w) + a5*cos(5*(t-b)*w) + b5*sin(5*(t-b)*w) +
                a6*cos(6*(t-b)*w) + b6*sin(6*(t-b)*w) - amp_shift_deg);
            
            // cout << theta_d_left_ankle << std::fixed << std::setprecision(3) << ";";

            rounded_theta = theta_d_left_ankle * 1000;
            itoa(rounded_theta, buf, 10);
            theta_str_n = buf + newline;
            writePort.write(theta_str_n);            
