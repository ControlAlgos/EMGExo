...
// Scale factor: 0.3 = 30% of full amplitude. Change this to adjust.
...
int main() {
    ...
    // Step 1: Soft Start to Target Position (scaled)
    double theta_f_right = (M_PI / 180) * 1.25 * SCALE * (a0 + a1 + a2 + a3 + a4 + a5 + a6 - amp_shift_deg);
    double theta_f_left = (-M_PI / 180) * 2 * SCALE * (a0 + a1 * cos(-b * w) + b1 * sin(-b * w) + a2 * cos(-2 * b * w) +
    ...
    double ktheta_f_right = (-M_PI / 180) * 7 * SCALE * (ka0 + ka1 + ka2 + ka3 + ka4 + ka5 + ka6 - amp_shift_deg);
    ...
    while (running_for <= run_time_ms) {
        ...
        if (duration_cast<milliseconds>(t_now - t_prev) >= dt_ms) {
            ...
            //Radial/Ulnar Deviation (scaled)
            double theta_d_right = (M_PI / 180) * 1.25 * SCALE * (a0 + a1*cos(t*w) + b1*sin(t*w) +
            ...
            //Flexion/Extension (scaled)
            double theta_d_left = (-M_PI / 180) * 2 * SCALE * (a0 + a1*cos((t-b)*w) + b1*sin((t-b)*w) +
            ...
            //Pronation/Supination (scaled)
            double ktheta_d_right = (-M_PI / 180) * 7 * SCALE * (ka0 + ka1*cos(t*w) + kb1*sin(t*w) +
