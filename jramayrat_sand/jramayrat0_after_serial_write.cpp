...
int main() 
{
    ...
    std::cout << "Creating serial file" << std::endl;
    serial_file.open("serial_file.csv");
    serial_file << "motor info" << std::endl;

    int dt = 2; // this is an int; units in ms
    int step;  // keeps track of the number of commands sent
    auto dt_ms = milliseconds(dt);

    std::cout << "Is this working" << endl;

    // ========== Set up for Serial communication ========== //
    // Write definition and setup
    SerialPC writePort;

    /// @note: Check which COM port your computer uses (e.g. Device Manager in Windows)
    // char write_port_num[] = "COM10"; // CP210x USB to UART Bridge (COM10) Option on computer
    ...
    /// @note(jramayrat): Not sure why this only had 1 argument originally:
    // writePort.setup(baudrate);  //set to COM3; change COM port inside SerialPC::setup() method
    ...
    {    //run_again is a c string, so it's a char array
        ...
        while(step <= set_homing_dur / (dt*0.001)) {
            ...
            if(duration_cast<milliseconds>(t_now - t_prev) >= dt_ms) {
                ...
                std::cout << "enc_data1[i1]: " << enc_data1[i1] << std::endl;
                serial_file << enc_data1[i1] << std::endl;
                // std::cout << "enc_data1[i2]: " << enc_data1[i2] << std::endl;
        ...
        std::cout << "======= THESE ARE DATA FOR 1st MOTOR ID =======\n";
        for (int i = 0; i < i1; i++) {
           std::cout << enc_data1[i] << ",\n";
        }

        std::cout << endl;

        std::cout << "======= THESE ARE DATA FOR 2nd MOTOR ID =======\n";
        for (int i = 0; i < i2; i++) {
           std::cout << enc_data2[i] << ",\n";
        }
