...
#include "C:\Users\Mo Sharifi\Documents\PlatformIO\Projects\Exoskeleton code\serial-rw\serialComm_WriteAndRead\json.hpp"
...
int main() {
    ...
    */
    ...
    float last_theta_d = 0;  // Define a variable to store the last theta_d when paused
    ...
    while(running_for <= run_time_ms) {
            ...
            if(elapsed >= dt_ms) {
                ...
                // Reads JSON file from local directory
                ifstream motor( "C:\\Users\\Mo Sharifi\\Desktop\\\\motor.json" );       
                if ( ! motor.is_open() ) {                 
                cout <<" Failed to open" << endl;
                    ...
                    // Make sure not to set frequency, w, before 1.8 or past 2.4
                    ...
                    std::cout << motorCase << endl;
                    //Paused state of motor case
                    if (motorCase == 0) {
                        // Record the start time
                        auto startPause = std::chrono::high_resolution_clock::now();
                        theta_d = last_theta_d;
                        // Use the last_theta_d when motorCase is 0
                        cout << theta_d << std::fixed << std::setprecision(4) << ",";  // DEBUG
                        // Formatting data to send over the COM port
                        rounded_theta = theta_d * 10000;
                        itoa(rounded_theta, buf, 10);
                        theta_str_n = buf + newline;
                        writePort.write(theta_str_n);
                        // Record the paused time
                        auto endTimePause = std::chrono::high_resolution_clock::now();
                        auto pause_time = std::chrono::duration_cast<std::chrono::seconds>(endTimePause - startPause);
                        auto pause_time_ms = std::chrono::duration<float, std::milli>(pause_time);
                        run_time_ms += pause_time_ms;
                    }
                    if (motorCase == 1) {
                        // Right hip
                        ...
                        // Deleted other trajectories for now, can add back in later
